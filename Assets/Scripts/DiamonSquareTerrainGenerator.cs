...
public class DiamonSquareTerrainGenerator : MonoBehaviour
{
    ...
    void CreateTerrain()
    {
        ...
        for(int i = 0; i <= mDivisions; i++)
        {
            ...
            {
                mVerts[i * (mDivisions + 1) + j] = new Vector3(-halfSize+j*divSize, 0f, halfSize-i*divSize);
        ...
        if(mVerts[mDivisions].y < 0)
        {
            mVerts[mDivisions].y = 0;
        }
        ...
        for (int i = 0; i <= mDivisions; i++)
        {
            for (int j = 0; j <= mDivisions; j++)
            {
                if(mVerts[i * (mDivisions + 1) + j].y > maxTerrainHeight)
                {
                    maxTerrainHeight = mVerts[i * (mDivisions + 1) + j].y;
                }
                if(mVerts[i * (mDivisions + 1) + j].y < minTerrainHeight)
                {
                    minTerrainHeight = mVerts[i * (mDivisions + 1) + j].y;
                }
                float height = Mathf.InverseLerp(minTerrainHeight, maxTerrainHeight, mVerts[i * (mDivisions + 1) + j].y);
                colors[i * (mDivisions + 1) + j] = gradient.Evaluate(height);
            }
        }
    ...
    void DiamondSquare(int row, int col, int size, float offset)
    {
        ...
        if(mVerts[topLeft + halfSize].y < 0f)
        {
            mVerts[topLeft + halfSize].y = 0f;
        }
        mVerts[mid - halfSize].y = (mVerts[topLeft].y + mVerts[botLeft].y + mVerts[mid].y) / 3 + Random.Range(-offset, offset);
        if(mVerts[mid - halfSize].y < 0f)
        {
            mVerts[mid - halfSize].y = 0f;
        }
        ...
        if(mVerts[botLeft + halfSize].y< 0f)
        {
            mVerts[botLeft + halfSize].y = 0f;
        }
    }
}
